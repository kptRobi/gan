...
def train_step(self, batch):
    real_images = batch
    fake_images = self.generator(tf.random.normal((batch, 128, 1)), training=False)
    ...
    with tf.GradientTape() as discrriminator_tape:
        ...
        concatenated_labels = tf.concat([tf.zeros_like(real_images_predictions), tf.ones_like(fake_images_predictions)],
                                        axis=0)

        # noise TODO wypróbować różne wartości
        noise_real = 0.15 * tf.random.uniform(tf.shape(real_images_predictions), minval=-1, maxval=1)
        noise_fake = 0.15 * tf.random.uniform(tf.shape(fake_images_predictions), minval=-1, maxval=1)
    ...
    with tf.GradientTape() as generator_tape:
        generated_images = self.generator(tf.random.normal((batch, 128, 1)), training=True)
